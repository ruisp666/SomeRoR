...
class Assignment
  ...
  def create_user(params)
      ...
      #p1= User.new
      #p1.username= params(username)
      #p1.password_digest = params(password_digest)
  ...
  def find_allusers(offset, limit)
      ...
      return User.order(updated_at: :asc).limit(limit).offset(offset)
  end
  ...
  def find_alllists(offset, limit)
     ...
     return TodoList.order(list_due_date: :desc).limit(limit).offset(offset)
  ...
  def find_todolist_byname(name)
      ...
      return TodoList.where(list_name: name)
  end     
